...
ext_cols = [col for col in ['EXT_SOURCE_1', 'EXT_SOURCE_2', 'EXT_SOURCE_3'] if col in X.columns]
print(f"\n✨ Available EXT_SOURCE columns: {ext_cols}")

# 6. ext_source_weighted - WEIGHTED combination (most important feature!)
if 'EXT_SOURCE_2' in X.columns and 'EXT_SOURCE_3' in X.columns:
    # EXT_SOURCE_2 is the strongest predictor, weight it highest
    X['ext_source_weighted'] = (
        0.6 * X['EXT_SOURCE_2'].fillna(0) +
        0.4 * X['EXT_SOURCE_3'].fillna(0)
    )
    print("   ✅ Created ext_source_weighted (0.6*ES2 + 0.4*ES3)")

# 7. ext_source_mean = mean of available EXT_SOURCE columns
if len(ext_cols) > 0:
    X['ext_source_mean'] = X[ext_cols].mean(axis=1)
else:
    X['ext_source_mean'] = 0

# 8. ext_source_min = min of available EXT_SOURCE columns
if len(ext_cols) > 0:
    X['ext_source_min'] = X[ext_cols].min(axis=1)
else:
    X['ext_source_min'] = 0

# 9. ext_source_std = std of available EXT_SOURCE columns
if len(ext_cols) > 0:
    X['ext_source_std'] = X[ext_cols].std(axis=1)
else:
    X['ext_source_std'] = 0

# 10. days_id_published_yr = DAYS_ID_PUBLISH / -365
X['days_id_published_yr'] = X['DAYS_ID_PUBLISH'] / -365

# 11. phone_change_yr = DAYS_LAST_PHONE_CHANGE / -365
...
new_features = ['age_years', 'employment_years', 'income_per_person', 'annuity_to_credit',
                'credit_to_income', 'ext_source_weighted', 'ext_source_mean', 'ext_source_min', 
                'ext_source_std', 'days_id_published_yr', 'phone_change_yr']

for col in new_features:
    if col in X.columns and X[col].isnull().sum() > 0:
        X[col] = X[col].fillna(X[col].median())

print(f"\n✨ Created {len([f for f in new_features if f in X.columns])} new features:")
